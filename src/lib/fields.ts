...
export const FIELD_TYPE_META: Record<FieldType, FieldTypeMeta> = {
  ...
  attachment: { type: "attachment", label: "Attachment", icon: "Paperclip", defaultOptions: { maxSizeMB: 25, allowedMimeTypes: [] } },
  user: { type: "user", label: "User", icon: "User", defaultOptions: { allowMultiple: false } },
  ...
  count: { type: "count", label: "Count", icon: "Hash", computed: true },
  formula: { type: "formula", label: "Formula", icon: "FunctionSquare", computed: true, defaultOptions: { expression: "" } },
  duration: { type: "duration", label: "Duration", icon: "Timer", defaultOptions: { unit: "seconds" } },
  button: { type: "button", label: "Button", icon: "MousePointerClick", computed: true, defaultOptions: { label: "Open", url: "" } },
